...
import model.AIPlayer1;
import model.AIPlayer2;
...
public class PokerGameController {
    ...
    public PokerGameController(PokerGame gameView, String name) {
        ...
        for (int i = 0; i < ais.length; i++) {
            if (rand.nextInt(300) < 100) {
                ais[i] = new AIPlayer("AI_" + (i + 1), 100);
            } else if (rand.nextInt(300) < 200
                    && rand.nextInt(300) >= 100) {
                ais[i] = new AIPlayer1("Bettor AI_" + (i + 1), 100);
            } else {
                ais[i] = new AIPlayer2("Coward AI_2" + (i + 1), 100);
            }
            players[i + 1] = ais[i];
        }
        ...
        dealSound = new AudioClip("File:./src/main/res"
                + "/shuffle-cards-1.wav");
    }

    /**
     * PokerGameController's constructor overloaded
     * @param  gameView The PokerGame object to interact with
     * @param  name     The human's name
        ...
        for (int i = 0; i < ais.length; i++) {
            if (rand.nextInt(100) < 50) {
                ais[i] = new AIPlayer("AI_" + (i + 1), chips);
                    ...
                    && rand.nextInt(300) >= 100) {
                ais[i] = new AIPlayer1("Bettor AI_" + (i + 1), chips);
            } else {
                ais[i] = new AIPlayer2("Coward AI_" + (i + 1), chips);
            }
            players[i + 1] = ais[i];
        }
